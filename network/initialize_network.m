...
% for testing repulsion - swimmer is very close to 4 nodes
m = [4 12 12]  ;
x = linspace(0.5,4,m(1)); % cube 20 um wide
...
% m = [3 20 20];
% x = linspace(0.65,1.2,m(1)); % cube 20 um wide
% y = 2*linspace(-1,1,m(2));
% z = 2*linspace(-1,1,m(3));

% single plane of points around body
% m = [4 20 20]  ;
% x = linspace(0.65,2.75,m(1)); % cube 20 um wide
% y = 4*linspace(-1,1,m(2)) / 3;
% z = 4*linspace(-1,1,m(3))  / 3;
...
%                  i.e. + f_s is tension of spring, - f_s is compression of
%                  spring
            else
...
[s,e] = plot_mesh(Mesh,2); s(1).FaceAlpha = 0.5;
light
