function [A_a_1,A_b_1,A_c_1,  A_a_2,A_b_2,A_c_2,  A_a_3,A_b_3,A_c_3,  A_a_4,A_b_4,A_c_4,    A_freeswim_rows,A_freeswim_cols,A_motor_torque,A_force,A_torque,  debug_info] = matrix_assembly_mex(Mesh,matrix_props,assembly_input)
...
%only produces submatrices A_a_(1-4), A_b_(1-4), A_c_(1-4) of full A to avoid mex error
%due to A being too big.  As a workaround, we insert A_a, A_b, A_c into full A in
...
%amount of time.  _a, _b, _c usually refer to x, y, z components but in
%case of free-slip BC, instead correspond to the 3 constraints U.n =
%(U+rxOmega).n, f.s_1 = 0, f.s_2 = 0 where n is the mesh normal and s_1,
%s_2 the two tangent vectors.  A is first split into 3 a, b, c
% horizontal subparts and then further split into a number of vertical
% sections to further reduce chances of Coder memory limitation being hit
...
% BC_type for each submesh is (currently) either 1 for no-slip or 2 for
% free-slip
if any( assembly_input.BC_type == 2 ) % for free-slip BC, will need to find matrix columns (i.e., x y z traction) going with each vert
    tot_verts = Mesh(end).indices.glob.unq_bounds.vert(2 ); % total # global verts
...
if matrix_props.n_col * matrix_props.n_col*3 < assembly_input.performance.numels_max  %good enough to just use Ax, Ay, Az
    A_a_1 = zeros(matrix_props.n_col,matrix_props.n_col*3);  A_b_1 = A_a_1;  A_c_1 = A_a_1;  %only includes tractions for all cases
    A_a_2 = [];  A_b_2 = [];  A_c_2 = [];   A_a_3 = [];  A_b_3 = [];  A_c_3 = [];   A_a_4 = [];  A_b_4 = [];  A_c_4 = [];
...
elseif ceil(matrix_props.n_col * matrix_props.n_col*3 / 2)  < assembly_input.performance.numels_max  %what if we split Ax, Ay, Az into two, are they small enough then?
    A_a_1 = zeros(matrix_props.n_col,ceil(matrix_props.n_col*3 / 2));  A_b_1 = A_a_1;  A_c_1 = A_a_1;  %only includes tractions for all cases
    A_a_2 = zeros(matrix_props.n_col,matrix_props.n_col*3 - ceil(matrix_props.n_col*3 / 2));  A_b_2 = A_a_2;  A_c_2 = A_a_2;  %only includes tractions for all cases
    A_a_3 = [];  A_b_3 = [];  A_c_3 = [];   A_a_4 = [];  A_b_4 = [];  A_c_4 = [];
    ...
    A_a_1 = zeros(matrix_props.n_col,ceil(matrix_props.n_col*3 / 4));  A_b_1 = A_a_1;  A_c_1 = A_a_1;  %only includes tractions for all cases
    A_a_2 = zeros(matrix_props.n_col,ceil(matrix_props.n_col*3 / 4));  A_b_2 = A_a_2;  A_c_2 = A_a_2;  %only includes tractions for all cases
    A_a_3 = zeros(matrix_props.n_col,ceil(matrix_props.n_col*3 / 4));  A_b_3 = A_a_3;  A_c_3 = A_a_3;  %only includes tractions for all cases
    A_a_4 = zeros( matrix_props.n_col ,matrix_props.n_col*3 - ceil(matrix_props.n_col*3 / 4)*3 );  A_b_4 = A_a_4;  A_c_4 = A_a_4;  %only includes tractions for all cases
    if assembly_input.performance.verbose
...
if ~assembly_input.skip_traction_integrals %if this is on, we skip the entire core part of A since we can copy it from the previous run (if we are doing freeswim now and just did forced)
    ...
    for i_mesh_vert = 1:length(Mesh) %only parfor over one submesh at a time to avoid load balancing problem when skip_rigid_integrals = true
        ...
        parfor (col_i = Mesh(i_mesh_vert).indices.glob.unq_bounds.vert(1) : Mesh(i_mesh_vert).indices.glob.unq_bounds.vert(2) , assembly_input.performance.nthreads)  %rows of A, i.e. vertices
            ...
            A_x_temp = zeros(matrix_props.n_col*3,1); A_y_temp = zeros(matrix_props.n_col*3,1); A_z_temp = zeros(matrix_props.n_col*3,1);
            ...
            for elem_i = 1:tot_elems  %elem_i is a global index
                ...
                A_x_temp(col_inds) = A_x_temp(col_inds) + submat(:,1);
                A_y_temp(col_inds) = A_y_temp(col_inds) + submat(:,2);
                A_z_temp(col_inds) = A_z_temp(col_inds) + submat(:,3);
            ...
            switch assembly_input.BC_type(i_mesh_vert)
                case 1
                    A_a_temp = A_x_temp;  A_b_temp = A_y_temp;  A_c_temp = A_z_temp;
                case 2  
                    normal = Mesh(i_mesh_vert).normals(local_vert,:); % unit normal vector at current vert
                    tangents = squeeze(Mesh(i_mesh_vert).tangents(local_vert,:,:)); % 2 orthogonal unit tangent vectors at current vert
                    
                    A_a_temp = A_x_temp * normal(1) + A_y_temp * normal(2) + A_z_temp * normal(3);  % u.n at current vert
                    A_b_temp = zeros(size(A_a_temp);  A_c_temp = zeros(size(A_a_temp);
                    traction_inds = find( vertind_cols == col_i ); %the 3 consecutive column inds corresponding to col_i vert
                    A_b_temp(traction_inds) = tangents(:,1);
                    A_c_temp(traction_inds) = tangents(:,2);     
            end
            ...
            if numels < assembly_input.performance.numels_max  %good enough to just use Ax, Ay, Az
                ...
                A_a_1(col_i,:) = A_a_temp;
                A_b_1(col_i,:) = A_b_temp;
                A_c_1(col_i,:) = A_c_temp;
            ...
            elseif ceil(numels / 2)  < assembly_input.performance.numels_max  %what if we split Ax, Ay, Az into two, are they small enough then?
                ...
                A_a_1(col_i,:) = A_a_temp(1:n);
                A_b_1(col_i,:) = A_b_temp(1:n);
                A_c_1(col_i,:) = A_c_temp(1:n);
                
                A_a_2(col_i,:) = A_a_temp(n+1 : end);
                A_b_2(col_i,:) = A_b_temp(n+1 : end);
                A_c_2(col_i,:) = A_c_temp(n+1 : end);
                ...
                A_a_2(col_i,:) = A_a_temp(n+1 : n * 2);
                A_b_2(col_i,:) = A_b_temp(n+1 : n * 2);
                A_c_2(col_i,:) = A_c_temp(n+1  :  n * 2);
                
                A_a_3(col_i,:) = A_a_temp(n*2+1 : n * 3);
                A_b_3(col_i,:) = A_b_temp(n*2+1 : n * 3);
                A_c_3(col_i,:) = A_c_temp(n*2+1 : n * 3);
                
                A_a_4(col_i,:) = A_a_temp(n*3+1 : end);
                A_b_4(col_i,:) = A_b_temp(n*3+1 : end);
                A_c_4(col_i,:) = A_c_temp(n*3+1 : end);
...
if strcmp(assembly_input.problemtype,'freeswim')
    ...
    % move unknown U1 U2 U3 W1 W2 W3 from RHS of boundary integral equations into the last 6 columns of A
    parfor (col_i = 1:matrix_props.n_col , assembly_input.performance.nthreads) % loop over collocation points (i.e. global verts)
        %  for col_i = 1:matrix_props.n_col
        
        i_mesh_vert = i_mesh_verts(col_i);
        local_vert = local_verts(col_i);

        x_col = Mesh(i_mesh_vert).verts(local_vert,:)'; % coords of current collocation point (i.e. vert)
        r = x_col - refpoint; % lever arm from torque balance reference point to collocation point
