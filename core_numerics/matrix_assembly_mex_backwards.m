function [A,A_motor_torque,A_force,A_torque,debug_info] = matrix_assembly_mex(Mesh,matrix_props,assembly_input,skip_rigid_integrals)
...
delta = matrix_props.n_col + 1;  % reverses order of parfor to fix load balancing problem when skip_rigid_integrals = true
parfor (col_i = 1:matrix_props.n_col, assembly_input.nthreads)  %rows of A, i.e. vertices
   % for col_i = 1:matrix_props.n_col
     % col_i / matrix_props.n_col
    AE = [];   NV = []; FL = logical([]); %stop parfor complaints about temp variables
    
    [i_mesh_vert, local_vert] = global2local(delta - col_i, Mesh, vert_range);     %compute i_mesh_vert and local_vert, indices to current vertex's submesh and local vertex index
    
    %integrand_constants = [];  
    integrand_constants = struct('eps2',assembly_input.accuracy.eps2,'x_col',Mesh(i_mesh_vert).verts(local_vert,:)');
    %integrand_constants.eps2 = assembly_input.eps2;
    %integrand_constants.x_col = Mesh(i_mesh_vert).verts(local_vert,:)';
    ...
    for elem_i = 1:tot_elems  %elem_i is a global index
        ...
        if strcmp(assembly_input.problemtype,'freeswim')
            ...
            if skip_rigid_integrals %integrals over elements on same object as current collocation point,
    ...
    Ax(delta - col_i,:) = Axtemp;
    Ay(delta - col_i,:) = Aytemp;
    Az(delta - col_i,:) = Aztemp;
    
    if assembly_input.debug_mode
        Integral(:,:,delta - col_i,:) = integral;
        Abserror(:,:,delta - col_i,:) = abserror;
        Numevals(delta - col_i,:) = numevals(1,:);
        Exitflag(delta - col_i,:) = exitflag(1,:);
    end
    
    
    
end %parfor over vertices i.e. collocation points
...
if strcmp(assembly_input.problemtype,'freeswim')
    ...
    switch assembly_input.tail.motorBC
        case 'torque'
            A(matrix_props.n_col*3+7,  1:matrix_props.n_col*3) = motor_orientation(1)*tailtorque(1,:) + motor_orientation(2)*tailtorque(2,:) + motor_orientation(3)*tailtorque(3,:);    
        case 'freq'
    ...
    parfor (col_i = 1:matrix_props.n_col , assembly_input.nthreads)
        %  for col_i = 1:matrix_props.n_col
    ...
    A(3*((1:matrix_props.n_col)-1)+1,  matrix_props.n_col*3+[1 5 6]) = temp1;
    A(3*((1:matrix_props.n_col)-1)+2,  matrix_props.n_col*3+[2 6 4]) = temp2;
    A(3*((1:matrix_props.n_col)-1)+3,  matrix_props.n_col*3+[3 4 5]) = temp3;
    
    if strcmp(assembly_input.tail.motorBC,'torque')  %hard coded to assume that Mesh(2) is tail
        A(3*( (1:matrix_props.n_col)-1 )+1,  matrix_props.n_col*3+7) = temp4;
        A(3*( (1:matrix_props.n_col)-1 )+2,  matrix_props.n_col*3+7) = temp5;
        A(3*( (1:matrix_props.n_col)-1 )+3,  matrix_props.n_col*3+7) = temp6;
    end
