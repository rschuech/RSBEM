...
folder = 'C:\Users\rudi\Desktop\RD\meshes_no_offset_2_1.5\';

folder = 'C:\Users\rudi\Desktop\RD\meshes_8x_thin_tail_length\';
...
for f = 1:length(Files{1})
    ...
    if ~all(SizeOK(f,:))  %all files aren't present and good size
        disp(['Skipping index ',num2str(f),', file missing or size no good']);
        continue
    end
...
%         ph = plot3(pts_fit(:,1),pts_fit(:,2),pts_fit(:,3),'k.');
    ...
    for hc = 1:length(hair_cases)
        ...
        if ismember(hair_case,names)
            ...
            dist_tols = [    0.1   0.25 0.5 1 2 ];  %parallel_2
            %          dist_tol = 0.04;  %parallel_1
            %          dist_tol = 0.2;
            max_tries = 10;  % 5 is too small, sometimes fails
            tries = zeros(size(dist));
            parameter_tol = 5E-2;
            %     tol = 0.1;
            ...
            parfor (vert_i = 1:length(u_sol), input.performance.nthreads)
                ...
                %   for vert_i = 1:length(u_sol)
                %  disp([hair_case,' vert ',num2str(vert_i)]);
                %  vert_i / Mesh(ni).n_vert
                vert = Mesh(ni).verts(vert_i,:);
                %   vert = Surface_vis(ni).verts(vert_i,:);
                if  ~isempty(last_solution.(hair_case))
