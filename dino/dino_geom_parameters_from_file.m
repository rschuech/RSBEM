...
parameters_loaded = dlmread(parameters_file);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if fat_tail
    geom.tail.radius = 0.75;   % fat
else
    geom.tail.radius = 0.15; % regular
end
...
geom.tail.t_transition = parameters_loaded(1,1) ; % with offset
...
geom.tail.translation = parameters_loaded(2,:)';  % with offset
...
% geom.transverse.R = 5.7; %deeply recessed hairs

%geom.R = 15.7;  big groove I think
geom.transverse.d = 0;
%geom.w = 0.1;
geom.transverse.w = 0.85;  %4
geom.transverse.rf = 3.2 / 2;  % Lasse's amplitude value is peak-to-peak
...
% geom.transverse.b = 2.5 ; %deeply recessed hairs
...
geom.transverse.u_max = parameters_loaded(3,1);  % with offset
% geom.transverse.u_max = 6.283185307179586 ; %no offset


geom.transverse.v_max = 1;
%     geom.shift = 8.7;
% geom.transverse.shift = body_height0 * 0.825 * 1.25;
geom.transverse.shift = parameters_loaded(4,1);  %gotten from Salome command line, need to add shift_x to Parameters.txt     with offset
...
geom.hairs.Coplanar_Hairs.h_max = coplanar_length;

geom.hairs.Normal_Top_Hairs.h_min = -normal_length;  
geom.hairs.Normal_Top_Hairs.h_max = 0;  %top is actually negative side

geom.hairs.Normal_Bottom_Hairs.h_min = 0;  
geom.hairs.Normal_Bottom_Hairs.h_max = normal_length;  %bottom is actually positive side
