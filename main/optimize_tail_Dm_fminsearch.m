...
% solutions_file = 'Dm_solutions.mat'; %stores optimized tail parameters so far across all racks
...
% sweep_file = ['sweep_Dm_',rack,'_',num2str(num),'.mat'];
sweep_file = 'Dm_few_redo.mat';

clear X Fval Flag Output Time

temp = load(['C:\Users\rudi\Desktop\RD\sweeps\',sweep_file]);
bodies_sweep = temp.sweep.bodies(inds,:);
guesses_sweep = [temp.sweep.amp(inds,:) temp.sweep.lambda(inds,:) ];
...
lock_file_name = ['results_lock_Dm_',num];
results_file_name =   ['Results_Dm_few_redo_',num,'.mat'];
dump_folder = 'C:\Users\rudi\Desktop\RD\Dm_dumps\';

% results_file_name = 'Results_.mat';

sweep_tempfile = ['tempsweep_Dm_',num2str(num),'.mat'];
...
%     end
    ...
    objfun = @(tail) - 1/scaling.obj * optimization_wrapper_Dm(AR1, AR2, tail(1)*scaling.range(1)+scaling.mean(1), tail(2)*scaling.range(2)+scaling.mean(2), max_arclength, results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
