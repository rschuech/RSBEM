...
sweep_file = ['sweep_efficiency_radially_ordered_',rack,'_',num2str(num),'.mat'];
...
results_file_name =   ['Results_efficiency_primary_sweep',num,'.mat'];
...
% CFD01 35:47
%%
for sw = 19:20 %:size(bodies_sweep,1)     %25:71
...
%         'pollmethod','GPSPositiveBasis2N', 'PollOrderAlgorithm', 'random', 'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',5E-4);
      options = optimoptions('patternsearch','cache','on','cachetol',1E-4,'display','iter','InitialMeshSize',2^(-2), 'AccelerateMesh',true,'maxfunctionevaluations',500, ...
        'pollmethod','GPSPositiveBasis2N',  'UseCompletePoll', false, 'MaxMeshSize' , 2,'MeshTolerance',0.0075,'PollOrderAlgorithm','Success');
