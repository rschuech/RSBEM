...
for sweep_i = 1:length(Inputs)
    ...
    switch input.bugtype
        ...
        case 'dino'
            ...
            %  submeshes = temp(logical(input.potatohead));  %only load submeshes that will be simulated - not sure if this will break anything for old cases.....
        ...
        switch input.problemtype
            case 'forced'
                ...
                for i = 1:length(flowcases)
                    ...
                    BCs(i).forced = set_forced_BCs(U0, Omega0,flowcase); %defines BCs struct with U and Omega vectors
                    ...
                    [fcoeffs_temp] = friction_coeffs(forces,BCs(i).forced);  %computes translational and rotational friction coeffs
...
%                                 y0 = [Mesh(1).refpoints(:,1); [0 0 0 ]' ];
        ...
        clear temp* x y
        temp = who;
        temp = setdiff(temp,{'A_recycle','A_force_recycle','A_torque_recycle', 'Inputs', 'A0', 'debug_info','Results','temp','global_Results'}); %don't want to save A_recycle in the .mat file
         allvars = whos;
