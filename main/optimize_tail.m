...
results_file_name =   ['Results_',num,'.mat'];
...
% CFD01 35:47
%%
for sw = 37:size(bodies_sweep,1)     %25:71
...
%     end
    ...
    objfun = @(tail) - optimization_wrapper(AR1, AR2, tail(1), tail(2), tail(3), results_file, lock_file_name, results_file_name , sweep_tempfile);
...
%         'pollmethod','GPSPositiveBasis2N', 'PollOrderAlgorithm', 'random', 'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',5E-4);
      options = optimoptions('patternsearch','cache','on','cachetol',1E-4,'display','iter','InitialMeshSize',2^(-5), 'AccelerateMesh',true,'maxfunctionevaluations',500, ...
        'pollmethod','GPSPositiveBasis2N',  'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',1E-3);
