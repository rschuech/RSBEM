function [Dm] = optimization_wrapper_Dm(AR1,AR2,Amp,Lambda,max_arclength ,results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder)
...
if exist(results_file,'file')
    ...
    if ~isempty(Results)
        ...
        if found && ~isempty(Results(ind).Dm)
            Dm = Results(ind).Dm;
            return
        end
    end
...
try
    Dm = Results(results_ind).Dm;  %should still be floating around from main.m
catch
    Dm = NaN;  % probably due to tail intersecting body and most of main.m not running
end
