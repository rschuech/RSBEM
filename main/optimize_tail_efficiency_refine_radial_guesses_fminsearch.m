...
sweep_file = ['efficiency_smooth_refined_CFD02.mat'];
...
lock_file_name = ['results_lock_efficiency_refined_',num];
results_file_name =   ['Results_efficiency_refined_optimum_',num,'.mat'];
dump_folder = 'C:\Users\rudi\Desktop\RD\efficiency_dumps_refined_optimum\';

% results_file_name = 'Results_.mat';

sweep_tempfile = ['tempsweep_efficiency_refined_',num2str(num),'.mat'];
...
%     end
    ...
    objfun = @(tail) - 1/scaling.obj * optimization_wrapper_efficiency_freeswim_only(AR1, AR2, tail(1)*scaling.range(1)+scaling.mean(1), tail(2)*scaling.range(2)+scaling.mean(2), tail(3)*scaling.range(3)+scaling.mean(3), results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
