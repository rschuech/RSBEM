...
%           inputs.paths.datfolder = 'E:\Hull\dinoflagellate\hairsheet3\';  
    ...
    case {'CFD01','CFD02','CFD03','CFD04'}
        ...
        inputs.paths.datfolder = 'C:\Users\rudi\Desktop\RD\Oscar_meshes\';   %location of .dat T6 mesh file(s)
        ...
        rack = getenv('computername');
        inputs.paths.results_file = ['C:\Users\rudi\Desktop\RD\Results\results_Oscar_unrefined',rack,'.mat'];
        distcomp.feature( 'LocalUseMpiexec', false ); %fixes bug in parallel toolbox as of 2015a
        inputs.paths.dumpfolder = 'C:\Users\rudi\Desktop\RD\Oscar_unrefined_dumps\';
...
switch inputs.bugtype
    case 'bacteria'
       % load ../sweeps/Oscar_sweep.mat  %contains all new AR1, AR2, amp, lambda, nlambda
...
switch inputs.bugtype
    case 'bacteria'
        inputs.accuracy.integration_tol.area.abstol = 1E-4 ;
    case {'dino', 'sheet'}
...
switch inputs.bugtype
    case 'bacteria'
        inputs.accuracy.integration_tol.volume.abstol = 1E-4 ;
    case 'dino'
...
switch inputs.bugtype
    case 'bacteria'
        inputs.accuracy.integration_tol.centroid.abstol = 1E-4 ;
    case 'dino'
...
%     inputs.body.V = 1;  %volume, microns^3
   inputs.body.V  = mat2cell(V,1,ones(1,length(V)));
    % %equivalent sphere radius
%     inputs.body.sphererad = (inputs.body.V*3/4/pi)^(1/3);
    inputs.body.sphererad = mat2cell(  (V*3/4/pi).^(1/3) , 1, ones(1,length(V)));
...
%         inputs.tail.nlambda = num2cell(1.49 * [ 0.875  1  1.125 ]);  %number of helix wavelengths in tail
        ...
        inputs.tail.lambda = lambda;
         inputs.tail.amp = amp;
         inputs.tail.nlambda = nlambda;
         
%         inputs.tail.lambda = mat2cell([repmat(lambda],1,ones(1,length(V)));
%         inputs.tail.amp = mat2cell([amp],1,ones(1,length(amp)));
%         inputs.tail.nlambda = mat2cell([nlambda],1,ones(1,length(nlambda)));
...
for i = 1:length(Inputs)
    switch inputs.bugtype
        case 'bacteria'
          ...
          if isfield(Inputs(i).tail,'tail_type')
