...
solutions_file = 'pole2pole_efficiency_solutions.mat'; %stores optimized tail parameters so far across all racks
...
%normalized values range between around -1 , +1 
scaling.mean = [0.6 4.5 1.4]';  %amp lambda nlambda
scaling.range = [0.2 0.5 1]'; % values are typically mean +/- range
scaling.obj = 3E4;
%%
% bodies_sweep = [10 0.4];
...
results_file_name =   ['Results_efficiency_pole2pole_',num,'.mat'];
dump_folder = 'C:\Users\rudi\Desktop\RD\efficiency_dumps_pole2pole\';
...
%%
sw_vec = 8:8:size(bodies_sweep,1) ; % 1:2 - CFD04   3:4 - CFD01  5:6 - CFD02  7:8 - CFD03
c = 0;
for sw = sw_vec(4:end)  
    c = c+ 1;
...
%     guess = guesses_sweep(sw,:);
    ...
    if isempty(solutions) 
        guess  =   [0.6 4.5 1.375 ]; % halfway between sphere and AR1 10 AR2 0.4 optima
    else
...
%     end
    ...
    objfun = @(tail) - 1/scaling.obj * optimization_wrapper_efficiency_pole2pole(AR1, AR2, tail(1)*scaling.range(1)+scaling.mean(1), tail(2)*scaling.range(2)+scaling.mean(2), tail(3)*scaling.range(3)+scaling.mean(3), results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
%     constraintfun = @(tail) max_arclength_constraint(tail,max_arclength);
    
    
%     optionsNM = optimset('display','iter','TolFun',1E6,'TolX',0.0005);
      optionsNM = optimset('display','iter','TolFun',1E6,'TolX',0.005);
