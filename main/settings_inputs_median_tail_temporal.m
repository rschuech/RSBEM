...
%           inputs.paths.datfolder = 'E:\Hull\dinoflagellate\hairsheet3\';  
    ...
    case {'CFD01','CFD02','CFD03','CFD04'}
        ...
        inputs.paths.results_file = 'Results_median_tail_temporal.mat';
        inputs.sync_results = true;  %check all Results files for curre4nt geometry and overwrite / append if found (if false, only do this for local Results file)
        inputs.paths.lock_file = 'Results_lock_temporal'; % to avoid race condition between racks when writing to shared Results file
...
%         distcomp.feature( 'LocalUseMpiexec', false ); %fixes bug in parallel toolbox as of 2015a
        inputs.paths.dumpfolder = 'C:\Users\rudi\Desktop\RD\median_tail_temporal_dumps\';
...
switch inputs.bugtype
    case 'bacteria'
        load(['../sweeps/','median_obs_body_temporal_opt_tail_sweep.mat']);  %contains all new AR1, AR2, amp, lambda, nlambda
...
%                 inputs.tail.radius = 0.031018;
        ...
        inputs.tail.lambda = mat2cell([sweep.lambda(:)'],1,ones(1,length(sweep.lambda)));
        inputs.tail.amp = mat2cell([sweep.amp(:)'],1,ones(1,length(sweep.amp)));
        inputs.tail.nlambda = mat2cell([sweep.nlambda(:)'],1,ones(1,length(sweep.nlambda)));
