...
% load('C:\Users\rudi\Desktop\RD\coarse_optimization.mat');
% guesses_sweep = X';
 
guesses_sweep = [0.6545503198  4.626245299  1.742256685 ];


lock_file_name = ['results_lock_',num];
results_file_name =   ['Results_efficiency_redo_refined_test',num,'.mat'];
...
% CFD01 35:47
%%
for sw = 1:1%size(bodies_sweep,1)     %25:71
...
%     end
    ...
    objfun = @(tail) - optimization_wrapper_redo_timestepping(AR1, AR2, tail(1), tail(2), tail(3), results_file, lock_file_name, results_file_name , sweep_tempfile);
...
%         'pollmethod','GPSPositiveBasis2N', 'PollOrderAlgorithm', 'random', 'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',5E-4);
      options = optimoptions('patternsearch','cache','on','cachetol',1E-4,'display','iter','InitialMeshSize',2^(-4), 'AccelerateMesh',true,'maxfunctionevaluations',500, ...
        'pollmethod','GPSPositiveBasis2N',  'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',0.007,'PollOrderAlgorithm','Success');
