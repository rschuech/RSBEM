...
%   rack = 'CFD03';
solutions_file = 'temporal_SN_solutions_shorter_max_arclength.mat'; %stores optimized tail parameters so far across all racks
...
% sweep_file = ['SNR_smooth_CFD04.mat'];
% sweep_file = ['efficiency_pole2pole_coarse_sweep.mat'];
% temp = load(['C:\Users\rudi\Desktop\RD\sweeps\',sweep_file]);
% bodies_sweep = temp.sweep.bodies;

bodies_sweep = [1.5 0; 1.5 0.1; 1.5 0.2; 1.5 0.3; 1.5 0.4; 1.125 0.025; 1.125 0.1; 1.125 0.2; 1.125 0.3;];
% guesses_sweep =[ temp.sweep.amp  temp.sweep.lambda  ];  % nlambda is solved for based on max_arclength

clear X Fval Flag Output Time

scaling.mean = [0.4 5.5]';  %amp lambda
scaling.range = [0.4 4.6]';
scaling.obj = 3E4;
%%
% bodies_sweep = [10 0.4];
...
max_arclength = 10.472; % The longest Psi_swim-optimized flagellum in our morphospace corresponded to L = 10, K = 0.55, and was 10.472 microns.  
%%

lock_file_name = ['results_lock_temporal_SN_',num];
results_file_name =   ['Results_Temporal_SN_shorter_max_arclength_',num,'.mat'];
dump_folder = 'C:\Users\rudi\Desktop\RD\temporal_SN_dumps_shorter_max_arclength\';
...
% sw_vec = 4:4:size(bodies_sweep,1) ; % 1:2 - CFD04   3:4 - CFD01  5:6 - CFD02  7:8 - CFD03
sw_vec = 7:9;
c = 0;
for sw = sw_vec
    c = c+ 1;
  ...
  disp(['On opt sweep iter ',num2str(c),' out of ',num2str(length(sw_vec)),'      body AR1 ',num2str(AR1),'  AR2 ',num2str(AR2)]);
...
%     end
    ...
    objfun = @(tail) - 1/scaling.obj * optimization_wrapper_temporal_SN(AR1, AR2, tail(1)*scaling.range(1)+scaling.mean(1), tail(2)*scaling.range(2)+scaling.mean(2), max_arclength, results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
