...
scaling.mean = [0.6 4 1.4]';  %amp lambda
scaling.range = [0.4 3 0.18]';
scaling.obj = 200;
...
sweep_file = ['sweep_eff_uber_refined.mat'];

clear X Fval Flag Output Time

load(['C:\Users\rudi\Desktop\RD\sweeps\',sweep_file]);

bodies_sweep = sweep.bodies;
guesses_sweep = [sweep.amp sweep.lambda sweep.nlambda];
switch rack
    case 'CFD03'
        bodies_sweep = bodies_sweep(1:184,:);
        guesses_sweep = guesses_sweep(1:184,:);
    case 'CFD04'
            
        bodies_sweep = bodies_sweep(185:end,:);
        guesses_sweep = guesses_sweep(185:end,:);
end

% bodies_sweep = [temp.new_sweep.AR1' temp.new_sweep.AR2'];
%  guesses_sweep = [temp.new_sweep.amp' temp.new_sweep.lambda' temp.new_sweep.nlambda'];

dump_folder = 'C:\Users\rudi\Desktop\RD\efficiency_dumps_uber_refined\';

lock_file_name = ['results_lock_',num];
results_file_name =   ['Results_efficiency_uber_refined_sweep',num,'.mat'];

% results_file_name = 'Results_.mat';

sweep_tempfile = ['eff_tempsweep_',num2str(num),'.mat'];
...
% CFD01 35:47
%%
for sw = 154:size(bodies_sweep,1)     %25:71
...
%     end
    ...
    objfun = @(tail) - 1/scaling.obj * optimization_wrapper_redo_timestepping_medium(AR1, AR2, tail(1)*scaling.range(1)+scaling.mean(1), tail(2)*scaling.range(2)+scaling.mean(2), tail(3)*scaling.range(3)+scaling.mean(3), results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
