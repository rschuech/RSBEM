...
% for Fore-Aft S/N runs only, hardcode path slope as x-axis since actual
% "swimming path" is some random direction at extremely slow speed
 Results(results_ind).path_slope = [1 0 0]';
 % again assume swimming dir is x-axis for pole separation calc
 fits.converged.speed = 1E5;
fits.line.speed = 1E5;  %arbitrary but needs to be non-zero for code to work
fits.line.intercept = [0 0 0]';
fits.line.slope = [1 0 0]';
 Results(results_ind).pole_separation = calc_pole_separation(input, Metadata, fits);
