...
guesses_sweep = temp.sweep_subset(:,3:5);
dump_folder = 'C:\Users\rudi\Desktop\RD\efficiency_nlambda_redo\';

lock_file_name = ['results_lock_',num];
results_file_name =   ['Results_efficiency_primary_sweep_nlambda_redo',num,'.mat'];
...
guess = guesses_sweep(sw,3);
amp = guesses_sweep(sw,1);   lambda = guesses_sweep(sw,2);
...
%     end
    ...
    objfun = @(nlambda) - optimization_wrapper_redo_timestepping_medium(AR1, AR2, amp, lambda, nlambda, results_file, lock_file_name, results_file_name , sweep_tempfile, dump_folder);
...
%         'pollmethod','GPSPositiveBasis2N', 'PollOrderAlgorithm', 'random', 'UseCompletePoll', false, 'MaxMeshSize' , 1,'MeshTolerance',5E-4);
      options = optimoptions('patternsearch','cache','on','cachetol',1E-4,'display','iter','InitialMeshSize',2^(-2), 'AccelerateMesh',true,'maxfunctionevaluations',500, ...
        'pollmethod','GPSPositiveBasis2N',  'UseCompletePoll', false, 'MaxMeshSize' , 2,'MeshTolerance',0.0075  / 10, 'PollOrderAlgorithm','Success');
