%   Inputs = Inputs(1:228); % Inputs = Inputs(13:end);
%  Inputs = Inputs(229:456);  %Inputs = Inputs(27:end);    Inputs = Inputs(155:end);

% Inputs = Inputs(457:684);  % Inputs = Inputs(229:end);
%Inputs = Inputs(685:end);  % Inputs = Inputs(28:end);
...
for sweep_i = 1:length(Inputs)
    ...
    if ( strcmp(input.bugtype,'bacteria') && length(Mesh) > 1 )  || (  strcmp(input.bugtype,'dino') && strcmp(input.problemtype,'forced')  )
        ...
        if is_intersected
            ...
            temp = load(input.paths.results_file);
            ...
            Results(end).Avg_Speed = NaN;
            Results(end).diff_cutoff = NaN;
            Results(end).t_convergence = NaN;
            Results(end).Avg_Omega = NaN;
            Results(end).Motor_Torque = NaN;
            Results(end).Avg_Power = NaN;  %one of these should be constant and the other varying unless I eventually implement a constant power motor condition
            Results(end).Torque_eff = NaN;
            Results(end).Power_eff = NaN;
            
            Results(end).Adj_Speed = NaN;
            Results(end).forced = [];
            Results(end).D = [];
            Results(end).tau_a = NaN;
            Results(end).Dm = NaN;
            Results(end).error_angle = NaN;
            
            Results(end).pole_separation = NaN;
            Results(end).taxis.temporal.SN = NaN;
            Results(end).taxis.temporal.ability = NaN;
            Results(end).taxis.fore_aft.SN = NaN;
            Results(end).taxis.fore_aft.ability = NaN;
    ...
    if strcmp(input.problemtype,'freeswim')  && input.do_timestepping
        ...
        %   * = not fully converged
        %  ** = not converged, possibly wierd path
        
        % 48.3253       thin tail    body-transverse-tail
        % 33.5477       thin tail    body-transverse
        % 4.2068        thin tail    body-tail             **
        % 145.9052      thin tail    transverse-tail
        % 130.8474      thin tail    transverse
        % 145.9052      thin tail    tail
        
        % 52.8833       fat tail     body-transverse-tail
        % 33.5535       fat tail     body-transverse
        % 4.8764        fat tail     body-tail             *
        % 146.3202      fat tail     transverse-tail
        % 130.8778      fat tail     transverse
        % 170.8550      fat tail     tail                  **
        ...
        if strcmp(input.bugtype,'bacteria')
            ...
            while ~worked
                ...
                try
                    ...
                    Results(end).lambda = input.tail.lambda;
                    Results(end).nlambda = input.tail.nlambda;
                    Results(end).Avg_Speed = fits.converged.speed;
                    Results(end).diff_cutoff = fits.converged.diff_cutoff;
                    Results(end).t_convergence = fits.T( fits.line.speed == fits.converged.speed);
                    Results(end).Avg_Omega = avg_omega;
                    Results(end).Motor_Torque = input.tail.motor_torque;
                    
                    Results(end).Avg_Power = Results(end).Avg_Omega * Results(end).Motor_Torque;  %one of these should be constant and the other varying unless I eventually implement a constant power motor condition
                    
                    Results(end).Torque_eff = 8*pi*input.constants.mu*input.body.sphererad^2*Results(end).Avg_Speed / Results(end).Motor_Torque;
                    
                    Results(end).Power_eff = 6*pi*input.constants.mu*input.body.sphererad*Results(end).Avg_Speed^2 / Results(end).Avg_Power;
                    
                    Results(end).Adj_Speed = sqrt( Results(end).Avg_Speed.^2 .* input.constants.power  ./ Results(end).Avg_Power);
                    Results(end).forced = solutions_temp;  %fcoeffs and forced simulation metadata e.g. imposed translation, rotation speeds
                    Results(end).D = D;  %Brownian diffusivities
                    ...
                    parameterized_line.intercept = fits.line.intercept(:,ind);
                    parameterized_line.slope = fits.line.slope(:,ind);
                    parameterized_line.speed = fits.line.speed(ind);
                    
                    [pole_separation] = line_projection_distance([tail_pole_x tail_pole_y 0]', [head_pole_x head_pole_y 0]', parameterized_line);
                    ...
                    if ~isempty(D)
                        Results(end).tau_a = 1/ (D.rotation.diffusivity(2,2) +  D.rotation.diffusivity(3,3));  %timescale for loss of orientation around first principle axis, which is hopefully close to swimming axis
                        Results(end).Dm = 1/3 * Results(end).Adj_Speed^2 * Results(end).tau_a;
                        ...
                        swimming_axis = slope / sqrt(sum(slope.^2));   %3 x 1 each between -1, 1
                        Results(end).error_angle = acos(dot(rotation_axis, swimming_axis)) * 180/pi;  %how big is erroneous angle between rotational diffusion principle axis and avg swimming axis
                        
                        Results(end).pole_separation = pole_separation;
                        
                        Results(end).taxis.temporal.SN = Results(end).Adj_Speed * sqrt(Results(end).tau_a);
                        Results(end).taxis.temporal.ability = Results(end).taxis.temporal.SN * Results(end).Adj_Speed;
                        
                        Results(end).taxis.fore_aft.SN = Results(end).pole_separation *  sqrt(Results(end).tau_a);
                        Results(end).taxis.fore_aft.ability =  Results(end).taxis.fore_aft.SN * Results(end).Adj_Speed;
                        
                        Results(end)
                        Results(end).taxis.temporal.SN
                        Results(end).taxis.temporal.ability
                        
                        Results(end).taxis.fore_aft.SN
                        Results(end).taxis.fore_aft.ability
                        
                    end %else, these stay as initialized NaNs
            ...
            end  %while load/save of Results failed
