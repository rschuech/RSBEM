...
interpolant = []; rel_diff = [];

switch input.bugtype
    case 'bacteria'
        if length(y0) == 6 % freq motorBC
            y0_interp = [y0; 0];  %need this so y0 matches size of y, even for freq BC or dino bugtype when phase angle is known (just a formality for yprime() )
        else
            y0_interp = y0;
        end
    case {'dino', 'sheet'}
        y0_interp = [];  %not used for dino case
    ...
    for i = 1:length(solutions.phase)
        ...
        switch input.bugtype
            case 'bacteria'
                ...
                if ~isequal(y,y0_interp) %location and/or orientation has changed; this is not the first yprime calculation
                    ...
                    Mesh_temp(2) = rotateMesh(Mesh_temp(2),[ y(7) 0 0]');  %rotate tail by y(7) around x-axis
                    %then move all submeshes according to translation and rotation of body +
                    %tail
                    Mesh_temp = move_Mesh(Mesh_temp,[y(1:3) - Mesh_temp(1).refpoints(:,1); y(4:6)]);
        ...
        if input.output.interpolation.doplot  %update plot with each new interpolation point as it's computed
            ...
            if input.output.interpolation.doprint
                try
                    pp = pp + 1;
                    set(gcf,'Position',[ 1089         -37         816        1026]);
                    print('-dpdf',[input.paths.dumpfolder,input.paths.namebase.full,'_interpolation','_',num2str(pp),'.pdf'])
                    saveas(input.output.interpolation.fignum,[input.paths.dumpfolder,input.paths.namebase.full,'_interpolation','_',num2str(pp),'.png']);
                end
            end
    ...
    for f = 1:length(interp_fields)
        for i = 1:size(Solutions.(interp_fields{f}),2) %scalar components of this variable, e.g. U(1:3), Omega(1:3).  trig_interp only works for scalars currently
            ...
            if n_iter >= 2 %we have enough iterations to compare the last two
                ...
                rel_diff(n_iter).(interp_fields{f})(i) = RMS_fun_diff(fun1, fun2, phase_bounds);  %global phase angle should always be defined from 0 - 2*pi, and will contain entire cyclic motion
                c = c+1;
