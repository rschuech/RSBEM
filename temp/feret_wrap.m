...
fix_length_bug = false;  % true for old MicrobeJ versions with Feret - Length bug, false for new MicrobeJ that outputs correct Length
...
for im = 1:length(data) % images
    ...
    for c = 1:length(data(im).length)  %individual cells
        ...
        if ~fix_length_bug || Feret < (1 - donut_safety_factor) * max_Feret
            donut = false;
            if ~do_curv_correction
                ...
                if fix_length_bug
                    ...
                    if any(abs_errors > 1E-10)  % was 1E-6
