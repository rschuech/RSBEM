...
[VL_unity] = integration_wrapper(Mesh, 'unity', ...
    input.accuracy.mesh.integration_tol.area.abstol,...
    input.accuracy.mesh.integration_tol.area.reltol,...
    input.accuracy.mesh.integration_tol.area.maxevals,...
...
[VL_flux] = integration_wrapper(Mesh, 'flux', ...
    input.accuracy.mesh.integration_tol.volume.abstol,...
    input.accuracy.mesh.integration_tol.volume.reltol,...
    input.accuracy.mesh.integration_tol.volume.maxevals,...
    input.performance.nthreads, input.accuracy.triangle_integration); %compute integrals of F dot n * hS * phi ala divergence theorem, F hardcoded as [1/3 x  1/3 y  1/3 z] but other F can be added later
[VL_centroid] = integration_wrapper(Mesh, 'centroid', ...
    input.accuracy.mesh.integration_tol.centroid.abstol,...
    input.accuracy.mesh.integration_tol.centroid.reltol,...
    input.accuracy.mesh.integration_tol.centroid.maxevals,...
    input.performance.nthreads, input.accuracy.triangle_integration); %output:  row is elem, cols are phi = 1..6 for x, y, z centroid integrals
