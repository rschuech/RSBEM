...
switch getenv('computername')
    case 'UBERTOP'
        paths.outfolder = 'E:/Hull/Oscar_meshes_refined/';  %be sure to use / instead of \ here; regexprep won't work with \
...
if ~strcmp(geom.shape,'tail')
    for n = 1:length(V)
        geom(n).V = V(n) ;  %all body shapes will have this volume = 1 um^3
        geom(n).sphererad = sphererad(n);  %equivalent sphere radius
    end
end
...
%             
            ...
            mesh(c).maxsize = ( geom(c).radius ^ (0.4) ) * 0.6  * factor      / 2;  %0.8
