...
options = odeset('InitialStep',timestepping_tol.InitialStep ,'Refine',1,'RelTol',timestepping_tol.reltol,'AbsTol',timestepping_tol.abstol,'OutputFcn',@output_fun,'Events',@trespassing_events); 
    ...
    function [value,isterminal,direction] = trespassing_events(t_eval,y_eval)
        ...
        if t_eval ~= t_last_solve % if the time we are evaluating right now
            ...
            % we need to update the Mesh and Network to the current y, y
             Network0.E( ~isnan(stored_output.link_breakage_time) ) = 0;  % permanently set E = 0 each time a link breaks
            [Mesh, Network, mesh_node_parameters, y_swimmer, r] = update_Mesh_Network(Mesh0, Network0, t_eval, y_eval, mesh_node_parameters,index_mapping, assembly_input);
