function [value,isterminal,direction] = stop_function_orig2(t,y,y0,input)
...
[times_subset,inds] = sort(times_subset);  % ode45 sometimes goes backward in time, so need to fix data order
%dists = dists(inds);
speeds_subset = speeds_subset(inds);
...
pts = speeds_subset(times_subset >= t*(1-input.accuracy.timestepping.datafrac) & times_subset <= t); % take all speed values from last datafrac fraction of data but not after current time (in case ode45 went backwards and there are values after current time)

cv =  nanstd(pts) / nanmean(pts);   %coefficient of variation of above subset of speed points

value = max(input.accuracy.timestepping.minpts - find(times_subset <= t,1,'last'), 0) + (cv - input.accuracy.timestepping.cvtol);  % not converged until both min number of points has been reached and cv is less than cvtol
