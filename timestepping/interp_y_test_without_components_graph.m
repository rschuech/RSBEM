...
%     set(gcf,'position',[1163          61         751         939]);
    ...
    iter_text = annotation('textbox',[0 0 1 0.1],'string',['adaptive iteration # ',num2str(n_iter)],'fontsize',16,'fontweight','bold','linestyle','none',...
...
while n_iter < input.accuracy.interpolation.max_iter
    ...
    for f = 1:length(interp_fields)
        ...
        for i = 1:size(Solutions.(interp_fields{f}),2) %scalar components of this variable, e.g. U(1:3), Omega(1:3).  trig_interp only works for scalars currently
            ...
            if n_iter >= 2 %we have enough iterations to compare the last two
                ...
                rel_diff.dir(n_iter).(interp_fields{f})(i) = NaN;  %placeholder for first undefined relative difference
            end
    ...
    for f = 1:length(interp_fields)
        ...
        xlim(phase_bounds)
        if n_iter >= 2
            title(['relative difference = ',num2str(current_diffs.mag(f))]);
        end
...
%         set(gca,'fontsize',13);
        %                       if isvector(f)
        %                       ylabel(['|',labels{f},'|'],'fontsize',20,'fontweight','bold');
        %                       else
        ...
        for j = 1:size(solutions.(interp_fields{f}),2)
            ...
            %                 set(gca,'fontsize',13);
            if n_iter >= 2
                title(['relative difference = ',num2str(current_diffs.dir(pc))]);
            end
...
%             set(gca,'fontsize',13);
            ...
            %                         xlabel(['phase angle (rad)'],'fontsize',20);
            %                     end
            %                     ylabel([ '$\widehat{',labels{f},'}','_{',num2str(j),'}$'],'fontsize',20,'Interpreter','latex');
