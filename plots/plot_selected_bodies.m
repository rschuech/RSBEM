...
bodies = [1.4375 0;   10 0.15; 1 0; 4 0.7;];

bodies = [4 0.65];

fontsizes.axes = 12;  fontsizes.labels = 12;  fontsizes.title = 12;

figure(827); clf;  axes; hold on;
clear hans
...
%     ylim([Mesh(1).refpoints(2,1) Mesh(1).refpoints(2,1) + 1])
        xlim([(Mesh(1).Centroid(1) -2)    (Mesh(1).Centroid(1) + 2)])
    ylim([(Mesh(1).Centroid(2) - 2)     (Mesh(1).Centroid(2) + 2)])
    ...
    drawnow
    % pause
%     print('-dpng','-r300',['C:\Hull\STEM poster contest\images\',name(1:end-4),'.png']);
end