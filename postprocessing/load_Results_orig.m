...
cvals{31} = [1.2 1.5 2 3  4 6 8 10 12];
cvals{22} = [0.25 0.1:0.1:1 ];
...
for m =  [5]   % 1:length(Fs) % 5 6  18    22 for Gauss construction   31 for tau_a
    %     switch metrics{m}
    %         case 'uptake'
    %             limits = [1 10; 0 0.985];
    %         otherwise
...
%         [C,ch] = contour(unique(X),unique(Y),Z,cvals{m},'--','linewidth',0.001,'linecolor',contour_color);
        ...
        if strcmp(metrics{m},'eff')
            ...
            clabel_deterministic(C_temp,ch_temp,'fontsize',10,'LabelSpacing',220,'fontweight','bold','color',labels_color);
            drawnow
            clear eff_data;
            ...
            clabel_deterministic(C,ch,'fontsize',8,'LabelSpacing',1000,'color',labels_color);
% clabel(C,ch,'manual','fontsize',8,'LabelSpacing',200,'color',labels_color); %doesn't work, manual mode doesn't insert labels into contours nicely, just draws them on top of lines
            ...
            clabel_deterministic(C,ch,'fontsize',8,'LabelSpacing',870,'color',labels_color);
        end
