...
folder0 = 'C:\Hull\Results\fixed\';
folder = 'C:\Hull\Results\pole2pole\';

folders = {folder0; folder0; folder0; folder0; folder; folder0; folder0; folder0; folder0};
...
files = {'Uptake_Interpolant.mat',...
    ...
    'Results_efficiency_pole2pole_1_synced.mat',...   'Results_efficiency_uber_refined_sweep1_synced.mat'   'Results_efficiency_uber_refined_sweep1_synced.mat'
    'Results_Temporal_SN_smooth_1_synced',...  Results_Temporal_SN_radial_guesses_1_synced2_cleanedagain_synced2   'Results_Temporal_SN_radial_guesses_tryagain.mat'   'Results_Temporal_SN_cleanedup.mat'   Results_Temporal_SN_radial_guesses_1_synced2.mat
    'Construction_Ease_fixed_scaling.mat',...
    'Results_Tumbling_Ease.mat',...
    'Results_buckling_original_tail_config_refined.mat' };  %'Results_buckling_pole2pole2.mat'   'Results_buckling_original_tail_config_refined.mat'   be sure to also change last folders entry between folder / folder0 


Depvars = {'Uptake','Fore-Aft SN','Tumbling Ease','Dispersal','Swimming Efficiency', 'Temporal SN','Construction Ease', 'tau_a', 'Buckling'}; %,'tau_a','lambda_a','error_angle'};
Fnames = {'uptake',  'fore_aft',    'tumbling',           'Dm',                'eff',          'temporal',    'construction' ,'tau_a' ,'buckling'};
        ...
        for i = 1:size(AR1_AR2_unq,1) %each unique body shape
            ...
            switch depvar
                ...
                case 'Temporal SN'
                    ...
                    metric = temporal_SN;
                case 'Buckling'
                    buckling = [Results_subset.buckling];
                    temp = [buckling.torque_ratio];
                    metric = [temp.mean];
            end
...
% 0.7 0.8 0.9 0.95 0.97 0.99      1.01 1.015  1.016 1.0175 1.018 1.02
      ...
      label_spacings = [300  400 400 550  500  600       600   900    325       400         250     200 200   200];
...
cvals = { ...
    ...
    [1.25 1.5 1.75 2 3 4 5 7 10 15 20 ],... buckling 32
    [1.05 1.1:0.1:1.7],... tau_a SNR  33
...
for m =  [5]   % 1:length(Fs) % 5 6  18    22 for Gauss construction       7:15 for unweighted constructions      18:26 30 for weighted constructions
    % [7:12  18:23 30]  for final constructions set
    ...
    pcolor(X,Y,Z_orig ./ Z);  % first run original load_results for efficiency and save Z as Z_orig, then run this file
%      pcolor(X,Y,Z );
    shading interp;  colormap(parula(2000));  cb = colorbar;
    set(gca,'FontSize',12);
...
%               for cm = setdiff( 1:length(ch) , 10) % was fontsize 8
    ...
    switch figtype
        ...
        case 'panel'
            ...
            if m == 13 || m == 30
                ylabel(ax,'$$ \mathcal{K} $$ (curvature)','interpreter','latex','fontsize',14);
            end
            if m == 13 || m == 30
                xlabel(ax,'$$ \mathcal{L} $$ (elongation)','interpreter','latex','fontsize',14);
            end
            cb.FontSize = 12;
    ...
    switch figtype
        case 'individual'
            switch m
                case 5
                    d.MarkerSize = 5; d.MarkerEdgeColor = 'k'; d.MarkerFaceColor = 'k';
                    hold on;  [C,ch] = contour(unique(X),unique(Y),Z_orig ./Z ,[1 1.05 1.1 1.15 1.2 ],'--','linewidth',0.001,'linecolor',contour_color);
                    clabel_deterministic(C,ch,'fontsize',11,'color',labels_color,'labelspacing',700); 
                    xlabel(ax,'$$ \mathcal{L} $$ (elongation)','interpreter','latex','fontsize',fontsizes.labels);
                    ylabel(ax,'$$ \mathcal{K} $$ (curvature)','interpreter','latex','fontsize',fontsizes.labels);
                    cb.Label.String = '\fontsize{35}\Psi_{\fontsize{12}\it{swim}} \fontsize{35} / \fontsize{35}\Psi^{\fontsize{12}\it{pole-to-pole}}_{\fontsize{12}\it{swim}}';
