...
files = dir([folder,'Results_all.mat']);  % orig I've been using

% folder = 'E:\Hull\Results\';
% files = dir([folder,'Results_shrunk_synced.mat']);  % orig I've been using

folder = 'E:\Hull\Results\';
files = dir([folder,'Results_body_only.mat']);  % orig I've been using

folder = 'E:\Hull\Results\best\';
files = dir([folder,'Results_all_fixed2.mat']);  % orig I've been using

folder = 'E:\Hull\Results\fixed\';
files = dir([folder,'Results_efficiency_primary_sweep1_current_synced.mat']);

% files = dir([folder,'Results_efficiency_secondary_sweep1_synced.mat']);

files = dir([folder,'Results_Temporal_SN_radial_guesses_1_synced.mat']);

% files = dir([folder,'Results_efficiency_secondary_sweep1_synced2.mat']);

% files = dir([folder,'Results_Dm_1_synced.mat']);
...
for f = 1:length(files)
    ...
    for i = 1:length(results)
        if ~isfield(results,'Power_eff') || isempty(results(i).Power_eff)
...
for results_ind = 1:length(Results)
    if ~isfield(results,'Adj_Speed')
        Results(results_ind).Adj_Speed = NaN;
    end
    Results(results_ind).taxis.temporal.SN = Results(results_ind).Adj_Speed * (Results(results_ind).tau_a)^(3/2);
    if isempty(Results(results_ind).taxis.temporal.SN)
        Results(results_ind).taxis.temporal.SN = NaN;
    end
    
    Results(results_ind).taxis.temporal.ability = Results(results_ind).taxis.temporal.SN * Results(results_ind).Adj_Speed;
      if isempty(Results(results_ind).taxis.fore_aft.SN)
        Results(results_ind).taxis.fore_aft.SN = NaN;
    end
    
    Results(results_ind).Dm = 1/3 * Results(results_ind).Adj_Speed.^2 * Results(results_ind).tau_a;
    
    
    %Adj_Speed = sqrt( Results(results_ind).Avg_Speed.^2 .* input.constants.power  ./ Results(results_ind).Avg_Power);
    if ~isfield(results,'Avg_Omega')
        Results(results_ind).Avg_Omega = NaN;
    end
    if ~isfield(results,'Motor_Torque')
        Results(results_ind).Motor_Torque = NaN;
    end
    if ~isfield(Results,'temporal_SN') || isempty(Results(results_ind).temporal_SN)
        Results(results_ind).temporal_SN = NaN;
    end
    ratio = Results(results_ind).Avg_Omega / Results(results_ind).Motor_Torque; % should be constant for each shape
    
    power = 1E-3;  % new normalized power from settings_inputs
    
    % power = Omega * Torque
    % Omega = power / Torque
    % Torque = Omega / ratio
    % so....
    % Omega = power * ratio / Omega
    % Omega^2 = power * ratio
    
    Results(results_ind).Adj_Omega = sqrt(power * ratio);
    Results(results_ind).Adj_Torque = Results(results_ind).Adj_Omega / ratio;
...
%        Results(i).fore_aft_ability = fore_aft_ability(i);
        ...
        if isfield(Results,'tau_a_body')
            Results(i).tumbling = 1/Results(i).tau_a_body;
        end
        if isfield(Results,'avg_swimming_axis') && isempty(Results(i).avg_swimming_axis)
            Results(i).avg_swimming_axis = NaN(3,1);
            Results(i).path_angle = NaN;
        else
            if isfield(Results,'avg_swimming_axis')
                Results(i).path_angle = acos(dot(Results(i).avg_swimming_axis,[1 0 0]'))*180/pi;
            else
                Results(i).path_angle = acos(dot(Results(i).path_slope,[1 0 0]'))*180/pi;
            end
        end
    end
    ...
    if ~isfield(Results,'avg_swimming_axis')
        swimming_axis = Results(i).path_slope;
    else
        swimming_axis = Results(i).avg_swimming_axis;
    end
    if ~isfield(Results,'principle_axis_1') || isempty(Results(i).principle_axis_1)
        Results(i).principle_axis_1 = NaN(3,1);
    end
...
%     rotmat = rotate_arbitrary_vector( rotvec, angle);
    if ~isfield(Results,'rotational_diffusivity') || isempty(Results(i).rotational_diffusivity)
        Results(i).rotational_diffusivity = NaN(3,3);
    end
%     D_aligned = rotmat * Results(i).rotational_diffusivity * rotmat';  % that's how you rotate a tensor
    
%     if any(isnan(D_aligned(:))) || any(isnan(swimming_axis))
%         Results(i).tau_a = NaN;
%     else
% %         swimming_axis
...
Depvars = {'pole_separation','tau_a_body','tau_a','Power_eff','Dm', 'temporal_SN','fore_aft_SN','construction','tumbling','uptake'}; %,'tau_a','lambda_a','error_angle'};
titles = {'Swimming Speed','Dispersal','Temporal S/N','Fore-Aft S/N','Construction Ease','Tumbling Ease','Nutrient Uptake'};
names = {'Swimming Speed','Dispersal','Temporal SN','Fore-Aft SN','Construction Ease','Tumbling Ease','Nutrient Uptake'};
titles = Depvars;

% Depvars = {'tau_a','construction','uptake','Dbr'};
Depvars = {'Power_eff'};
Depvars = {'temporal_SN'};
% Depvars = {'Dm'};
...
% Depvars = {'Adj_Speed','Adj_Omega','Adj_Torque','fore_aft_SN'};
...
use_clims = false;
use_cvals = false;
extrap_method = 'linear';  % none usually, linear for new tails interpolation
...
for dv = 1:length(Depvars)
    ...
    if strcmp(depvar,'construction')
        ...
        if isempty(sphere_depvar)
            sphere_depvar = 1;
        end
        dep_var = best_depvars  / sphere_depvar;
        %     dep_var = 1./ dep_var;
    end
...
%              dep_var = best_nlambdas;
    ...
    F = scatteredInterpolant(standardized_subset(:,1), standardized_subset(:,2), dep_var(subset),method,extrap_method);
    ...
    if plot_datapts
        ...
        d = plot(X_Y_unq(subset,1),X_Y_unq(subset,2), '.','markersize',12,'markerfacecolor','none','markeredgecolor','k');
        switch Depvars{dv}
            %             case 'Power_eff'
    ...
    cvals = {unique([linspace(0.6,1.01,30) linspace(1.01,1.05,40)])};
    %     clims = [ 0.65 1.05];
    %     clims = [ 1 8];
    ...
    temp = vertcat(plot_data.mean_unweighted);
      hmeas = plot(temp(:,1),temp(:,2),'ko','markerfacecolor','k','markersize',2);
...
% area2 = curved_rod_area(X_Y_unq(:,1), X_Y_unq(:,2), 1);
...
% Z = 1./(mean_abs_curv2 .* area2);  Z = Z/max(Z(:));
Pareto_data.Construction_Ease.depvar = Z;

%%
[area, radius_curv1, radius_curv2] = curved_rod_area(X,Y,1);  % weight overall cost by surface area

Z = radius_curv1; % Z = Z/max(Z(:));
Z(isinf(Z)) = max(Z(~isinf(Z))) * 1E3;  Z = Z/max(Z(:));
F = scatteredInterpolant(standardized_X(~isnan(Z)), standardized_Y(~isnan(Z)), Z(~isnan(Z)),method,'none');
Pareto_data.Construction_Ease1.Z = Z;   Pareto_data.Construction_Ease1.F = F;

Z = radius_curv2;  %Z = Z/max(Z(:)); %can't normalize because max value is Inf for straight rods
Z(isinf(Z)) = max(Z(~isinf(Z))) * 1E3;  Z = Z/max(Z(:));
F = scatteredInterpolant(standardized_X(~isnan(Z)), standardized_Y(~isnan(Z)), Z(~isnan(Z)),method,'none');
Pareto_data.Construction_Ease2.Z = Z;   Pareto_data.Construction_Ease2.F = F;
...
for n = 1:length(racks)
    ...
    save(['E:\Hull\sweep_efficiency_CFD0',num2str(racks(n)),'_',num2str(reps(n)),'.mat'],'bodies','guesses');
...
for n = 1:length(racks)
    % n = 4;
    %primary sweep
    bodies = sweep_primary(n:length(racks):end,1:2);
    guesses = sweep_primary(n:length(racks):end,3:5);
    inds = randperm(size(bodies,1));
    bodies = bodies(inds,:);
    guesses = guesses(inds,:);
    %secondary sweep
    bodies2 = sweep_secondary(n:length(racks):end,1:2);
    guesses2 = sweep_secondary(n:length(racks):end,3:5);
    inds = randperm(size(bodies2,1));
    bodies2 = bodies2(inds,:);
    guesses2 = guesses2(inds,:);
    
    
    
    bodies_combined = [bodies; bodies2];
    guesses_combined = [guesses;  guesses2];
    save(['E:\Hull\sweep_efficiency_CFD0',num2str(racks(n)),'_',num2str(reps(n)),'.mat'],'bodies_combined','guesses_combined');
    %     allbods = [allbods; bodies];
end
%% timestepping redo
%   0.61746       4.2255       1.3435
...
for n = 1:length(racks)
    % n = 4;
    bodies = primary_sweep(n:length(racks):end,:);
    %     guesses = guesses0(n:length(racks):end,:);
    %     inds = randperm(size(bodies,1));
    %     bodies = bodies(inds,:);
    %     guesses = guesses(inds,:);
    save(['E:\Hull\sweep_efficiency_radially_ordered_CFD0',num2str(racks(n)),'_',num2str(reps(n)),'.mat'],'bodies');
    allbods = [allbods; bodies];
end
%% sweep is an N x 5 matrix of bodies and tails
...
for n = 1:length(racks)
    ...
    sweep_subset = sweep(n:length(racks):end , :);
    sweep_subset = sweep_subset(inds,:);
    
    
    save(['E:\Hull\sweep_efficiency_CFD0',num2str(racks(n)),'_',num2str(reps(n)),'.mat'],'sweep_subset');
    %     allbods = [allbods; bodies];
end

%% interpolate tail guesses for secondary sweep from primary sweep results
clear sweep
load('E:\Hull\secondary_sweep_bodies.mat','refined_only');
for i = 1:size(refined_only,1)
    
    limits = [1 10; 0 1];
    [standardized_body] = standardize(refined_only(i,:), limits);
    names = {'amp','lambda','nlambda'};
    clear parameters
    for n = 1:3
        
        parameters(n) = F_tails.(names{n})(standardized_body(1),standardized_body(2));
        
    end
    
    sweep(i,:) = [refined_only(i,:) parameters];
end
%%
racks = [1 2 3 4];
reps =  [1 1 1 1];
...
for n = 1:length(racks)
    ...
    %     bodies = bodies(inds,:);
    %     guesses = guesses(inds,:);
    save(['E:\Hull\secondary_sweep_temporal_SN_CFD0',num2str(racks(n)),'_',num2str(reps(n)),'.mat'],'sweep_subset');
