...
% maximum stability.
% [Q,R,E] = qr(M,0);  orig, no weights
[Q,R,E] = qr(bsxfun(@times,W(:),M),0);  %with weights ala suggestion on shatlab central comments for this file

polymodel.ModelTerms = modelterms;
% polymodel.Coefficients(E) = R\(Q'*depvar);  orig, no weights
polymodel.Coefficients(E) = R\(Q'*(W(:).*depvar));  %with weights ala suggestion on shatlab central comments for this file
