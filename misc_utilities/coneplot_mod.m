function varargout = coneplot_mod(varargin)
...
if quiv
    ...
    if exist('scale_vec','var')
        
        ui(end+1) = scale_vec.velocity(1)*s;  vi(end+1) = scale_vec.velocity(2)*s;  wi(end+1) = scale_vec.velocity(3)*s;
        cx(end+1) = scale_vec.position(1);  cy(end+1) = scale_vec.position(2);  cz(end+1) = scale_vec.position(3);
...
%     conesegments = 14;
    ...
    for i = 1:size(cx,1)
      ...
      if exist('scale_vec','var')
      h = patch('faces', faces(1:end-flen,:), 'vertices', verts, 'parent', cax);
    hs = patch('faces', faces(end-flen+1:end,:), 'vertices', verts, 'parent', cax);
      else
