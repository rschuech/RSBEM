function [cout, hand] = contour_modded(varargin)
    ...
    narginchk(1, Inf);
    [pvpairs, ~, ~, errmsg, warnmsg] = contourobjHelper_modded('parseargs', false, args{:});
    if ~isempty(errmsg)
        error(errmsg);
    end
    if ~isempty(warnmsg)
    ...
    contourobjHelper_modded('addListeners', cax, h);
