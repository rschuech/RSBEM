function varargout = contourobjHelper_modded(functionName, varargin)
...
function [bValid, zLevel, lab, lp, xc, yc, trot] = localContourLabelRenderParams(cs, i, k, labels, dummyExtent, xDir, yDir, axScaleXPos, axScaleYPos, bManual, p, labelSpacing, textList, getStartParam)
    ...
    if bManual
        ...
        bValid = (isempty(textList) || any(abs(zLevel - textList) / max(eps + abs(textList)) < 0.00001));  %.00001
    end
...
function [label_out, cut_out] = localPlaceLabels(contour,perform_cut,cax,text_parent,ptch_in,...
...
%
    ...
    if (strcmp(cax.XDir, 'reverse')), XDir = -1; else XDir = 1; end
    if (strcmp(cax.YDir, 'reverse')), YDir = -1; else YDir = 1; end
    ...
    while (ii<lCS),
        ...
        if (lp>0) && isfinite(lvl) && ...
                (isempty(v) || any(abs(lvl-v)/max(eps+abs(v)) < 0.00001)),
