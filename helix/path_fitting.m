...
%% line fit
warning('off', 'MATLAB:nearlySingularMatrix');  % fmincon occasionally warns with this on some tries
while fits.line.flag(end) <= 0 && fits.line.iters(end) < options.line.max_iter  %fit failed in some way
    ...
    [solution_line, rmse_line, fits.line.flag(end)] = fmincon(objfun,options.line.guess,[],[],[],[],options.line.lb,options.line.ub,confun,optimset('tolx',1E-8, 'tolfun', 1E-8, 'tolcon',1E-10,'maxfunevals',5E3, 'maxiter',5E3,'display','off','useparallel',false));
...
if isinf(fits.line.flag(end))  %first try for this T_interrogate
    ...
    else %first try for this T, try last solution if non-NaN
        if ~any(isnan(  [fits.line.intercept(:,end-1); fits.line.slope(:,end-1); fits.line.speed(end-1);] ) )
        options.line.guess = [fits.line.intercept(:,end-1); fits.line.slope(:,end-1); fits.line.speed(end-1);];
        else  %  try random guess
